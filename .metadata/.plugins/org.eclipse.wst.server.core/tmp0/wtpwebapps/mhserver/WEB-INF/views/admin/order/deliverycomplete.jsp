...
<script>
			...
			$.ajax({
				...
				success: function(result) {
					...
					$(result).each(function(index, item) {
						...
						} else if (item.delivery_status != "배송완료" || isEmpty(result)) {
			...
			$.ajax({
				...
				success: function(result) {
					...
					$(result).each(function(index, item) {							
						...
						} else if (item.delivery_status != "배송완료" || result.length < 1) {
	...
	$("#btn2").click(function() {
	    ...
	    $("#date2").val("");
	    $("#txt2").val("");
	    $(location).attr('href', "/mh/admin/order/deliverycomplete.do");
	});
