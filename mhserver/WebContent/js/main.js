...
$("#menubar > #searchbox")
    ...
    .keyup(function(){
        ...
        // 엔터 입력 시, 이미지 선택 시
        if(event.keyCode == 13 || $("#menubar > #searchimg").click()){
            // 검색어로 inputtext를 넣어줌
        }
...
$("#detailmenu").mouseleave(function(){
    $("#detailmenu").css({ "display":"none" })
})
...
var timer = setInterval(function () {
    if (noticeNum < 6) {
        $("#noticeMent").css({"display":"block"});
        $("#noticeMent").css({"transform":"translateY(-"+31*noticeNum+"px)"});
        noticeNum++;
    } else {
        noticeNum = 0;
        $("#noticeMent").css({"display":"none"});
        $("#noticeMent").css({"transform":"translateY(0px)"});
    }
